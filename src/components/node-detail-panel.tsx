...
import type { DocumentWithMemories, MemoryEntry, NodeDetailPanelProps } from '../lib/types';
...
export const NodeDetailPanel = memo<NodeDetailPanelProps>(({ node, onClose }) => {
          ...
          {isDocument ? (
            <>
              ...
              {(data as DocumentWithMemories).summary && (
                <div>
                  <span className="text-xs text-slate-400 uppercase tracking-wide">Summary</span>
              ...
              <div>
                <span className="text-xs text-slate-400 uppercase tracking-wide">Memory Count</span>
                <p className="text-sm text-slate-200 mt-1">
              ...
              {((data as DocumentWithMemories).url || (data as DocumentWithMemories).customId) && (
                <div>
                  <span className="text-xs text-slate-400 uppercase tracking-wide">URL</span>
                  <a
            ...
            <>
              <div>
                <span className="text-xs text-slate-400 uppercase tracking-wide">Memory</span>
                <p className="text-sm text-slate-200 mt-1">{(data as MemoryEntry).memory}</p>
                ...
                {(data as MemoryEntry).forgetAfter && (
                  <p className="text-xs text-slate-400 mt-1">
                    Expires:{' '}
                    {(data as MemoryEntry).forgetAfter
                      ? new Date((data as MemoryEntry).forgetAfter!).toLocaleDateString()
                      : ''}{' '}
              ...
              <div>
                <span className="text-xs text-slate-400 uppercase tracking-wide">Space</span>
                <p className="text-sm text-slate-200 mt-1">

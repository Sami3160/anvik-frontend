...
import type { MemoryGraphProps } from '../lib/types';
...
export const MemoryGraph = ({
  ...
  // Graph data
  const { nodes, edges } = useGraphData(data, selectedSpace, nodePositions, draggingNodeId);
  ...
  const checkAndLoadMore = useCallback(() => {
    if (isLoadingMore || !hasMore || !data?.documents || data.documents.length === 0) return;
    ...
    const visibleDocuments = data.documents.filter((doc) => {
      const docNodes = nodes.filter((node) => node.type === 'document' && node.data.id === doc.id);
  ...
  return (
    ...
    >
      ...
      {!isLoading && (!data || nodes.filter((n) => n.type === 'document').length === 0) && (
        <>{children}</>
      )}
